...
namespace plaster {
static const char* vs_color_quad = R"(
        #version 120
        void main() {
...
static const char* fs_color_quad = R"(
        #version 120
        void main() {
...
void Renderer::push_quad(glm::vec2 pos, glm::vec2 size, uint32_t color) {
...
void Renderer::push_rect_outline(glm::vec2 pos, glm::vec2 size, uint32_t color, float thickness) {
...
void Renderer::push_text(glm::vec2 pos, const char* text, uint32_t color, float scale) {
    // Placeholder: actual text rendering requires font atlasing
...
void Renderer::resize(uint16_t width, uint16_t height) { bgfx::reset(width, height, BGFX_RESET_VSYNC); }
